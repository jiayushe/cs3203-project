...
NextBipTConstraintLogic::NextBipTConstraintLogic(std::shared_ptr<KnowledgeBase::PKB> pkb,
...
bool NextBipTConstraintLogic::is_valid(const AssignmentMap& assignment_map) const {
    if (lhs.get_type() == Parser::StatementRefType::ANY &&
        ...
        for (auto const& statement : statements) {
            if (!statement->get_next_bip()->empty()) {
                return true;
            }
        }
        return false;
    ...
    if (lhs.get_type() != Parser::StatementRefType::ANY &&
        ...
        return !lhs_statement->get_next_bip()->empty();
    }

    if (lhs.get_type() == Parser::StatementRefType::ANY &&
        rhs.get_type() != Parser::StatementRefType::ANY) {
        auto rhs_statement = get_statement(assignment_map, rhs);
        return !rhs_statement->get_previous_bip()->empty();
    }

    auto lhs_statement = get_statement(assignment_map, lhs);
    auto rhs_statement_id = get_statement_id(assignment_map, rhs);

    auto lhs_statement_next_bip = lhs_statement->get_next_bip();
    return lhs_statement_next_bip->find(rhs_statement_id) != lhs_statement_next_bip->end();
}

std::unordered_set<std::string> NextBipTConstraintLogic::get_synonyms() const { return synonyms; }
