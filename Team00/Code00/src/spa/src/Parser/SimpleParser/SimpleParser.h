...
// SimpleParser for the SIMPLE language (converts tokens to AST).
    ...
    private:
        ...
        std::shared_ptr<SimpleNode> choice(const std::vector<std::function<std::shared_ptr<SimpleNode>()>> &parse_funcs,
        ...
        repeat(const std::function<std::shared_ptr<SimpleNode>(std::shared_ptr<SimpleNode>)> &parse_func,
