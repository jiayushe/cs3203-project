...
SimpleNode* build_test_node() {
    auto node = new SimpleNode(SimpleNodeType::ARITHMETIC, "+");
    node->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "x"));
    node->add_child(new SimpleNode(SimpleNodeType::ARITHMETIC, "*"));
    node->get_child(1)->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "y"));
    node->get_child(1)->add_child(new SimpleNode(SimpleNodeType::CONST_VALUE, "5"));
    return node;
}
...
TEST_CASE("Parser::SimpleNode") {
    ...
    SECTION("is_subtree") {
        ...
        SECTION("return false if the supplied node is not a subtree") {
            ...
            auto non_subtree_node = new SimpleNode(SimpleNodeType::ARITHMETIC, "+");
            non_subtree_node->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "x"));
            non_subtree_node->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "y"));
    ...
    SECTION("is_equal") {
        ...
        SECTION("return false if the supplied node is not equal (recursively)") {
            // Build an AST representing x + y
            auto different_node = new SimpleNode(SimpleNodeType::ARITHMETIC, "+");
            different_node->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "x"));
            different_node->add_child(new SimpleNode(SimpleNodeType::VAR_NAME, "y"));
